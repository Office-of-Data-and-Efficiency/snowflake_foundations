...
1. From the left navigation menu, click `Catalog`.
2. Click `Database Explorer`.
3. Click `IOP_APP_LAB_DB`.
4. Click `IRIS`.
5. Click `Create`.
6. Click `Table`.
7. Click `From File`.
...
If no schema is identified complete the following steps.
1. Choose:
...
- Here is where the schema format can reviewed and updated as needed by changing data types and column names.
